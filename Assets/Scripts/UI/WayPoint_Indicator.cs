...
public class WayPoint_Indicator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Faz a transição de branco para vermelho ao longo do tempo
        Color = Color.Lerp(Color.white, Color.red, timer / duration);
    }
}
...
class OffscreenMarkersCameraScript : MonoBehaviour
{
    ...
    private List<WayPoint_Indicator> _trackedObjects = new List<WayPoint_Indicator>();

    public void Register(WayPoint_Indicator om)
    {
    ...
    void OnGUI()
    {
        ...
        {
            WayPoint_Indicator marker = _trackedObjects[i];
            if (marker && marker.gameObject)
            {
                if (!IsVisible(marker.gameObject))
                {
                    ...
                    GUI.DrawTexture(ri, marker.Icon, ScaleMode.StretchToFill, true);
